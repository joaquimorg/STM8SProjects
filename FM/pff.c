...
#include "microsd.h"		/* Declarations of low level disk I/O functions */
#include "s65.h"
#include "stm8s.h"
...
)
{
	...
	// Put your code here
	res = MSD_ReadBytes(dest, sector, sofs, count);
...
)
{
	...
	if (!disk_readp(buf, sect, BS_FilSysType, 2) )
		if (LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
			return 0;
		
#if _FS_FAT32
	if (!disk_readp(buf, sect, BS_FilSysType32, 2))
		if (LD_WORD(buf) == 0x4146)	/* Check FAT32 */
			return 0;
#endif
...
)
{
	...
	u16 img_width, img_height;

	if (!fs) return FR_NOT_ENABLED;		/* Check file system */
	if (!(fs->flag & FA_OPENED))		/* Check if opened */
		return FR_NOT_OPENED;
	br = fs->fptr;
	dr = pf_read(buffer, 0x45, &rcnt);
	
	if ( dr != FR_OK ) return FR_DISK_ERR;
	
	if ( (buffer[0] != 0x42) && (buffer[0] != 0x4D) ) return FR_DISK_ERR;
	
	img_width = buffer[0x16];
	img_height = buffer[0x12];
	
	LCD_SetArea(x, y, x + img_width-1, y + img_height-1);
	LCD_SendCMD(RAMWR);
	fs->fptr = br;
	br = 0;
	remain = fs->fsize - fs->fptr;
	...
	while (btr)	{									/* Repeat until all data transferred */
		...
		rcnt = 512 - ((WORD)fs->fptr % 512);		/* Get partial sector data from sector buffer */
		if (rcnt > btr) rcnt = btr;
		dr = MSD_ReadBytes_bmp(fs->dsect, offs, rcnt);
		offs = 0;
		if (dr) goto fr_abort;
